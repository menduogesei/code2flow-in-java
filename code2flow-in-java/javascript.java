...
public class JavascriptHelper {
    private static final Logger logging = Logger.getLogger("JavascriptHelper");
    ...
    public static String resolve_owner(Map<String, Object> callee) {
        ...
        if ("MemberExpression".equals(objType)) {
            if (obj.containsKey("object") && ((Map<String, Object>) obj.get("property")).containsKey("name")) {
                return Utils.djoin(resolve_owner(obj), (String) ((Map<String, Object>) obj.get("property")).get("name"));
            }
        ...
        if ("NewExpression".equals(objType)) {
            ...
            return Utils.djoin((String) calleeObjObj.get("name"), (String) calleeObjProp.get("name"));
        }
        return OWNER_CONST.UNKNOWN_VAR;
    }
    ...
    public static Call get_call_from_func_element(Map<String, Object> func) {
        ...
        if ("MemberExpression".equals(type) && ((Map<String, Object>) callee.get("property")).containsKey("name")) {
            ...
            return new Call((String) ((Map<String, Object>) callee.get("property")).get("name"),
                    lineno(callee),
                    owner_token);
        }
        if ("Identifier".equals(type)) {
            return new Call((String) callee.get("name"), lineno(callee));
        }
        return null;
    }
    ...
    public static List<Call> make_calls(Object body) {
        ...
        for (Map<String, Object> element : walk(body)) {
            ...
            if ("CallExpression".equals(type)) {
                ...
                calls.add(new Call((String) ((Map<String, Object>) element.get("callee")).get("name"),
                        lineno(element)));
            }
        }
        return calls;
    ...
    public static List<Variable> process_assign(Map<String, Object> element) {
        ...
        if ("CallExpression".equals(init.get("type"))
            ...
            if (idMap.containsKey("name")) {
                ...
                String points_to_str = Utils.djoin(import_src_str, imported_name);
                List<Variable> vars = new ArrayList<>();
    ...
    public static List<Variable> make_local_variables(Object tree, Group parent) {
        ...
        if (parent instanceof Group && GROUP_TYPE.CLASS.equals(parent.group_type)) {
    ...
    public static List<String> get_inherits(Map<String, Object> tree) {
        ...
        if (tree.get("superClass") != null) {
            ...
            if (superClass.containsKey("name")) {
                ...
                inherits.add(Utils.djoin((String) obj.get("name"), (String) prop.get("name")));
            }
        }
        return inherits;
    }
    ...
    public static String get_acorn_version() {
        try {
            File currentFile = new File(JavascriptHelper.class.getProtectionDomain().getCodeSource().getLocation().toURI());
...
public class Javascript extends BaseLanguage {
    ...
    public static void assert_dependencies() {
        ...
        String version = JavascriptHelper.get_acorn_version();
    ...
    public static Object[] separate_namespaces(Map<String, Object> tree) {
        ...
        for (Map<String, Object> el : JavascriptHelper.children(tree)) {
    ...
    public static List<Node> make_nodes(Map<String, Object> tree, Group parent) {
        ...
        int line_number = JavascriptHelper.lineno(tree);
        List<Call> calls = JavascriptHelper.make_calls(this_scope_body);
        List<Variable> variables = JavascriptHelper.make_local_variables(this_scope_body, parent);
        Node node = new Node(token, calls, variables, parent, line_number, is_constructor);
        ...
        List<Node> subnodes = Utils.flatten(subnodesLists);
    ...
    private static Group nodeToGroup(Node node) {
        return new Group(node.token, "", "", new ArrayList<>(), node.line_number, null);
    }

    public static Node make_root_node(List<Map<String, Object>> lines, Group parent) {
        String token = "(global)";
        List<Call> calls = JavascriptHelper.make_calls(lines);
        List<Variable> variables = JavascriptHelper.make_local_variables(lines, parent);
        Node root_node = new Node(token, calls, variables, parent, 0);
        return root_node;
    }
    ...
    public static Group make_class_group(Map<String, Object> tree, Group parent) {
        ...
        int line_number = JavascriptHelper.lineno(tree);
        List<String> inherits = JavascriptHelper.get_inherits(tree);
        Group class_group = new Group(token, group_type, display_name, inherits, line_number, parent);

        for (Map<String, Object> node_tree : node_trees) {
            List<Node> new_nodes = make_nodes(node_tree, class_group);
            for (Node n : new_nodes) {
                class_group.add_node(n);
            }
        }
        return class_group;
    }
