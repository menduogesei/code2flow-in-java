...
class RubyHelper {
    ...
    public static Call get_call_from_send_el(List<Object> func_el) {
        ...
        if (owner != null && "new".equals(token)) {
            return new Call(owner);
        }
        return new Call(token, owner);
    }
    ...
    public static Variable process_assign(List<Object> assignment_el) {
        ...
        if ("send".equals(secondEl.get(0))) {
            ...
            return new Variable(varname, call);
        }
        return null;
    }
    ...
    public static List<Variable> make_local_variables(List<Object> tree_el, Group parent) {
        ...
        if (parent != null && parent.group_type == GROUP_TYPE.CLASS) {
            variables.add(new Variable("self", parent));
        }
        return variables;
    }
...
public class Ruby extends BaseLanguage {

    public static void assert_dependencies() {
        if (!Utils.is_installed("ruby-parse")) {
    ...
    public static List<Object> get_tree(String filename, LanguageParams lang_params) {
        ...
        try {
            ...
            Object conv = Utils.jsonToJava(jsonArray);
    ...
    public static Triple<List<List<Object>>, List<List<Object>>, List<List<Object>>> separate_namespaces(List<Object> tree) {
        ...
        for (List<Object> el : RubyHelper.as_lines(tree)) {
    ...
    public static List<Node> make_nodes(List<Object> tree, Group parent) {
        ...
        boolean is_constructor = token.equals("initialize") && parent.group_type == GROUP_TYPE.CLASS;
        List<List<Object>> tree_body = RubyHelper.get_tree_body(tree);
        ...
        List<Call> calls = RubyHelper.make_calls(flattenLines(separated.third));
        List<Variable> variables = RubyHelper.make_local_variables(flattenLines(separated.third), parent);
        Node node = new Node(token, calls, variables, parent, is_constructor);
        
        List<List<Node>> subnodesList = new ArrayList<>();
        for (List<Object> t : subnode_trees) {
            subnodesList.add(make_nodes(t, parent));
        }
        List<Node> subnodes = Utils.flatten(subnodesList);
    ...
    public static Node make_root_node(List<Object> lines, Group parent) {
        String token = "(global)";
        List<Call> calls = RubyHelper.make_calls(lines);
        List<Variable> variables = RubyHelper.make_local_variables(lines, parent);
        Node root_node = new Node(token, calls, variables, parent, false);
        return root_node;
    }
    ...
    public static Group make_class_group(List<Object> tree, Group parent) {
        ...
        List<String> inherits = RubyHelper.get_inherits(tree, body_trees);
        Group class_group = new Group(token, group_type, display_type, inherits, parent);
        ...
        for (Node node : class_group.nodes) {
            for (Node n : class_group.nodes) {
                node.variables.add(new Variable(n.token, n));
            }
        }
