...
public class Php {
    ...
    public static class NamespaceSeparationResult {
    ...
    public static Call get_call_from_expr(JSONObject func_expr) {
        ...
        if ("Expr_FuncCall".equals(nodeType)) {
            ...
            if (varObj.has("var")) {
                owner_token = Model.OWNER_CONST.get("UNKNOWN_VAR");
            } else {
        ...
        if (owner_token != null && "__construct".equals(token)) {
            return new Call(owner_token, lineno(func_expr), null, false);
        }
        Call ret = new Call(token, lineno(func_expr), owner_token, false);
        return ret;
    }
    ...
    public static List<Variable> make_local_variables(Object tree_el, Group parent) {
        ...
        if (parent != null && Objects.equals(parent.group_type, Model.GROUP_TYPE.get("CLASS"))) {
    ...
    public static String assert_dependencies() {
        assert Model.is_installed("php") : "No php installation could be found";
        ...
        ProcessResult pr = run_ast_parser(self_ref);
        String path = new File(Php.class.getProtectionDomain().getCodeSource().getLocation().getPath()).getParent();
        String assert_msg = String.format("Error running the PHP parser. From the `%s` directory, run " + "`composer require nikic/php-parser \"^4.10\"`.", path);
        assert pr.returnCode == 0 : assert_msg;
        return pr.output;
    }

    public static JSONArray get_tree(String filename, Object lang_params) {
        ProcessResult pr = run_ast_parser(filename);
        if (pr.returnCode != 0) {
    ...
    public static NamespaceSeparationResult separate_namespaces(Object tree) {
        ...
        for (int i = 0; i < treeArr.length(); i++) {
            ...
            if (nodeType.equals("Stmt_Function") || nodeType.equals("Stmt_ClassMethod") || nodeType.equals("Expr_Closure")) {
                ...
                NamespaceSeparationResult tup = separate_namespaces(children(el));
    ...
    public static List<Node> make_nodes(JSONObject tree, Group parent) {
        ...
        boolean is_constructor = token.equals("__construct") && Objects.equals(parent.group_type, Model.GROUP_TYPE.get("CLASS"));
        ...
        List<Call> calls = make_calls(sep.body);
        List<Variable> variables = make_local_variables(sep.body, parent);
        List<String> import_tokens;
        if (Objects.equals(parent.group_type, Model.GROUP_TYPE.get("CLASS")) && parent.parent != null && Objects.equals(parent.parent.group_type, Model.GROUP_TYPE.get("NAMESPACE"))) {
            import_tokens = Arrays.asList(djoin(parent.parent.token, parent.token, token));
        } else if (Objects.equals(parent.group_type, Model.GROUP_TYPE.get("NAMESPACE")) || Objects.equals(parent.group_type, Model.GROUP_TYPE.get("CLASS"))) {
            import_tokens = Arrays.asList(djoin(parent.token, token));
        } else {
            import_tokens = Arrays.asList(token);
        }
        Node node = new Node(token, calls, variables, parent, import_tokens, lineno(tree), is_constructor);
    ...
    public static Node make_root_node(JSONArray lines, Group parent) {
        String token = "(global)";
        int line_number = lines.length() > 0 ? lineno(lines.getJSONObject(0)) : 0;
        List<Call> calls = make_calls(lines);
        List<Variable> variables = make_local_variables(lines, parent);
        Node root_node = new Node(token, calls, variables, parent, null, line_number, false);
        return root_node;
    }
    ...
    public static Group make_class_group(JSONObject tree, Group parent) {
        ...
        String token = get_name(tree.getJSONObject("name"));
        ...
        List<String> import_tokens;
        if (display_type.equals("Class") && parent != null && Objects.equals(parent.group_type, Model.GROUP_TYPE.get("NAMESPACE"))) {
            import_tokens = Arrays.asList(djoin(parent.token, token));
        } else {
            import_tokens = Arrays.asList(token);
        }
        Group class_group = new Group(token, group_type, display_type, import_tokens, lineno(tree), parent, inherits);
        ...
        for (int i = 0; i < sep.nodes.size(); i++) {
            ...
            for (Node newNode : newNodes) {
                class_group.add_node(newNode, false);
            }
        }
        if (Objects.equals(group_type, Model.GROUP_TYPE.get("NAMESPACE"))) {
            class_group.add_node(make_root_node(new JSONArray(sep.body), class_group), false);
